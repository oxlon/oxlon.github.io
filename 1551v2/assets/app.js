...
function displayValue(value, exact=false) {
  ...
  if (exact || typeof value !== 'number') return String(exact?value:uiText(value));
...
const I18N = {
  en: {
    ...
    empty:'No rows match the current filters.', lang_button:'AZ interface',
  ...
  az: {
    ...
    empty:'Cari filtrlərə uyğun sətir yoxdur.', lang_button:'EN interface',
    search_title:'Cədvəl və ya metod axtar', search_placeholder:'Cədvəl, sənəd və dəftərçələr üzrə axtarış',
    strip_status:'Məlumat tərifləri', strip_note:'Modellərin mənbə tarixləri və tərifləri fərqlidir; göstərilən fərqlər təmiz metod müqayisəsi deyil. Bu təqdimat əvvəlki yoxlamanı yeniləmir və Nazirlik qəbulu vermir.'
  }
};
let lang = 'en';
...
const AZ_UI = {"Forecast explorer": "Proqnoz cədvəlləri", "Saved scenarios": "Saxlanmış ssenarilər", "Data library": "Məlumat kitabxanası", "Equations": "Tənliklər", "Terminology and column meanings": "Terminologiya və sütunların mənası", "Indicator": "Göstərici", "Indicator / model": "Göstərici / model", "Model": "Model", "Engine": "Model", "Scenario": "Ssenari", "Requirement": "Tələb", "Requirement or source module": "Tələb və ya mənbə modulu", "Year": "İl", "Year from": "Başlanğıc ili", "Year to": "Son il", "Unit": "Vahid", "Value": "Qiymət", "Source": "Mənbə", "Source cell": "Mənbə xanası", "Source worksheet": "Mənbə vərəqi", "Source file": "Mənbə faylı", "Source row": "Mənbə sətri", "Exact value": "Dəqiq qiymət", "Rows": "Sətirlər", "Records": "Qeydlər", "Years": "İllər", "Baseline": "Baza ssenarisi", "Difference": "Fərq", "Comparison": "Müqayisə", "Series": "Sıra", "Concept": "Göstərici", "Dataset": "Məlumat cədvəli", "Kind": "Növ", "Scope": "Əhatə", "Columns": "Sütunlar", "Original file": "İlkin fayl", "Contents and interpretation": "Məzmun və şərh", "Term": "Termin", "Meaning and interpretation": "Məna və şərh", "Displayed heading": "Göstərilən başlıq", "Original source field": "İlkin mənbə sahəsi", "Functional requirement": "Funksional tələb", "Required output": "Tələb olunan nəticə", "Required official inputs": "Tələb olunan rəsmi girişlər", "Approach and equations": "Yanaşma və tənliklər", "Validation and benchmarks": "Yoxlama və müqayisə meyarları", "Indicator comparisons": "Göstəricilərin müqayisəsi", "CPI worked example": "İQİ üzrə hesablama nümunəsi", "Accounts and requirements": "Hesablar və tələblər", "Sample": "Seçmə", "Specification": "Spesifikasiya", "Description": "Təsvir", "Name": "Ad", "Calculation status": "Hesablama statusu", "Observation status": "Müşahidə statusu", "Data status": "Məlumat statusu", "Open": "Aç", "Read worksheet": "Vərəqi oxuyun", "Original formula": "İlkin düstur", "Source type": "Mənbə növü", "Source cached value": "İlkin saxlanmış qiymət", "Saved baseline calculation": "Saxlanmış baza hesablaması", "Baseline evaluation status": "Baza hesablamasının statusu", "Partly evidenced": "Qismən sübut olunub", "Recorded source or calculation error": "Qeydə alınmış mənbə və ya hesablama xətası", "Source historical observation": "Mənbədəki tarixi müşahidə", "Recorded official observation": "Qeydə alınmış rəsmi müşahidə", "Calculated workbook forecast": "İş kitabında hesablanmış proqnoz", "Retained workbook expected value": "İş kitabında saxlanmış gözlənilən qiymət", "Calculated from supplied formulas": "Təqdim edilmiş düsturlarla hesablanıb", "Retained model forecast": "Saxlanmış model proqnozu", "Saved model forecast": "Saxlanmış model proqnozu", "Retained source historical input": "Saxlanmış tarixi mənbə girişi", "Functional requirements FR1–FR12": "Funksional tələblər FR1–FR12", "All requirements": "Bütün tələblər", "Terms used in this website": "Saytda istifadə edilən terminlər", "Column headings": "Sütun başlıqları"};
function uiText(value){return lang==='az'&&(typeof value==='string')?(AZ_UI[value]||value):value}
...
const modelMeta = id => (lang==='az'&&MODEL_META[id]?{...MODEL_META[id],short:{ministry_macro:'Nazirliyin makro modeli',ministry_caem:'Nazirliyin CAEM modeli',imf_articleiv_benchmark:'BVF istinadı'}[id]||MODEL_META[id].short}:MODEL_META[id]) || {short:id, cls:'benchmark', colour:'var(--benchmark)'};
...
const isArchiveScenario = g => !['baseline','Table1','acg_volume_plus10','oil_plus10','partner_minus1','wages_plus10'].includes(g.scenario_id);
  ...
  render() {
    ...
    for (const row of slice) {
      ...
      for (const col of o.columns) {
        ...
        else cell = escapeHTML(displayValue(friendlyValue(v,col.key), o.exact || /^(year|Year)$/.test(col.key) || /^(Year|İl)$/.test(col.label)));
...
const SECTIONS = [
  ...
  {path:'ministry-tables',label:()=>bilingual('Ministry worksheets','Nazirliyin vərəqləri'),group:'nav_workbench'},
  ...
  {path:'delivery-scope',label:()=>bilingual('Delivery scope and checks','Təhvilin əhatəsi və yoxlamalar'),group:'nav_evidence'},
...
function renderNav(current) {
 ...
 for(const group of [...new Set(SECTIONS.map(s=>s.group))]){
  ...
  for(const section of SECTIONS.filter(s=>s.group===group))html+='<a href="#/'+section.path+'"'+(current===section.path?' class="active" aria-current="page"':'')+'><span class="nav-dot"></span>'+escapeHTML(uiText(section.label()))+'</a>';
  html+='</details>';
 }
...
function head(title, lede, pathNote) {
  return '<div class="view-head"><h1>' + escapeHTML(uiText(title)) + '</h1>' + (lede ? '<p class="lede">' + escapeHTML(lede) + '</p>' : '') + (pathNote ? '<div class="path-note">' + escapeHTML(pathNote) + '</div>' : '') + '</div>';
}
...
function selectField(label, name, options, current, allLabel) {
  let html = '<div class="field"><label for="f-' + name + '">' + escapeHTML(uiText(label)) + '</label><select id="f-' + name + '" name="' + name + '">';
  ...
  for (const op of options) {
    ...
    html += '<option value="' + escapeHTML(val) + '"' + (String(val) === String(current) ? ' selected' : '') + '>' + escapeHTML(uiText(lab)) + '</option>';
  }
...
async function viewOverview(params, ticket) {
  ...
  html+='</tbody></table></div><div class="table-legend"><span><i class="legend-history"></i>'+(lang==='az'?'Tarixi müşahidə':'Historical observation')+'</span><span><i class="legend-expected"></i>'+(lang==='az'?'Gözlənilən':'Expected')+'</span><span><i class="legend-forecast"></i>'+(lang==='az'?'Proqnoz':'Forecast')+'</span><span>'+bilingual('Derived / IMF estimate / IMF projection: identified in each cell','Hesablanmış / BVF qiymətləndirməsi / BVF proqnozu: hər xanada göstərilir')+'</span><span>— '+(lang==='az'?'Mənbədə yoxdur':'Not supplied')+'</span></div>';
  html+='<p class="definition-note">'+bilingual('This is a selected indicator comparison, not the complete requirement inventory. Oil prices, population income, physical production, public/private GDP and gross-output/intermediate-consumption tables are available through the functional requirements and Ministry worksheets. A dash means no mapped record, not a zero.','Bu, seçilmiş göstəricilərin müqayisəsidir, bütün tələblərin siyahısı deyil. Neft qiymətləri, əhali gəlirləri, natural istehsal, dövlət/qeyri-dövlət ÜDM-i, ümumi buraxılış və aralıq istehlak cədvəlləri funksional tələblər və Nazirliyin vərəqləri vasitəsilə açılır. Tire uyğunlaşdırılmış qeydin olmadığını göstərir; sıfır demək deyil.')+' '+reviewLink('ministry-tables','Ministry worksheets','Nazirliyin vərəqləri')+' '+reviewLink('delivery-scope','Coverage and open issues','Əhatə və açıq məsələlər')+'</p>';
...
function scenarioLabel(id,model) { const g=catalogue.result_groups.find(g=>g.model_id===model&&g.scenario_id===id);const n=catalogue.result_groups.filter(g=>g.model_id===model).findIndex(g=>g.scenario_id===id)+1;if(g?.label){const base=g.label.replace(/ · [a-f0-9]{10}$/, '');const duplicate=catalogue.result_groups.filter(x=>x.model_id===model&&x.label===g.label).length>1;return base+(duplicate||id.startsWith('interactive_')?' · recorded case '+n:'');}return id==='baseline'?'Baseline':id.startsWith('interactive_')?'Saved source-input scenario · case '+n:id.replace(/_/g,' '); }
...
async function viewForecasts(params, ticket, scope) {
  ...
  loadingView();
  const showArchive=params.get('archive')==='1'||(scenario!=='*'&&scenario!=='baseline');
  const groups = catalogue.result_groups.filter(g => (showArchive||!isArchiveScenario(g)) && (model === '*' || g.model_id === model) && (scenario === '*' || g.scenario_id === scenario));
  ...
  const scenarios = [...new Set(catalogue.result_groups.filter(g => (showArchive||!isArchiveScenario(g))&&(model === '*' || g.model_id === model)).map(g => g.scenario_id))];
  ...
  html += '<div class="actions-row"><a class="btn" href="#/scenarios?archive=1">Diagnostic and archived calculations</a>'+(model==='ministry_macro'?'<a class="btn" href="#/ministry-tables">Ministry forecast worksheets</a>':'')+'<a class="btn" href="#/delivery-scope">Delivery scope and open issues</a></div>';
  ...
  function syncUrl() {
    ...
    if(showArchive)p.set('archive','1');
  ...
  function renderResult() {
    ...
    if (state.wide) {
      ...
      const columns = [
        {key:'model', label:'Model', sticky:true, get:r=>r.meta.model_id, render:r=>modelTag(r.meta.model_id)},
      ...
      const table = new DataTable({
        ...
        columns: [
          {key:'model_id', label:'Model', sticky:true, render:r=>modelTag(r.model_id)},
...
async function viewScenarios(params, ticket) {
  ...
  const scenarioGroups=catalogue.result_groups.filter(g=>showArchive||!isArchiveScenario(g));
  const diffGroups = scenarioGroups.filter(g => g.difference_parts && g.difference_parts.length);
  ...
  html += statusStrip(release);
  html += '<p class="definition-note">This page presents saved calculations. New input values must be run in the model environment before they can appear here.</p><div class="actions-row"><a class="btn" href="#/scenarios'+(showArchive?'':'?archive=1')+'">'+(showArchive?'Show baseline and declared examples':'Include diagnostic and archived calculations')+'</a></div>';
  html += '<div class="panel"><h2>Saved runs with baseline differences</h2><div class="table-scroll"><table class="data"><thead><tr><th><button type="button" class="th-btn">Model</button></th><th><button type="button" class="th-btn">Scenario</button></th><th class="num"><button type="button" class="th-btn">Rows</button></th><th><button type="button" class="th-btn"></button></th></tr></thead><tbody>';
  ...
  html += '<div class="panel"><h2>Available saved runs</h2><div class="table-scroll"><table class="data"><thead><tr><th><button type="button" class="th-btn">Model</button></th><th><button type="button" class="th-btn">Scenario</button></th><th class="num"><button type="button" class="th-btn">Records</button></th><th><button type="button" class="th-btn">Years</button></th><th><button type="button" class="th-btn"></button></th></tr></thead><tbody>';
...
async function viewEquations(params, ticket) {
  const tab = params.get('tab') === 'legacy' ? 'ministry' : params.get('tab') || 'published';
  loadingView();
  const release = await releaseP();
  if (!fresh(ticket)) return;
  ...
  html += statusStrip(release);
  const tabs = [['published','Econometric specifications'],['caem','CAEM methodology'],['native','Original workbook formulas'],['ministry','Ministry EViews equations (92)']];
  ...
  if (tab === 'published' || tab === 'ministry') {
    ...
    if (frs.length) bar += selectField('Requirement or source module', 'fr', frs.map(fr=>({value:fr,label:requireLabel(fr)})), frFilter, t('all'));
    bar += '<p class="definition-note">FR1–FR12 are specification references. FR13, FR1B and FR5B are supplementary source-module labels, not additional Ministry requirements. The earlier comparison module does not replace the supplied Ministry equations or linked workbooks.</p>';
    ...
    const table = new DataTable({
      ...
      columns: [
        ...
        {key:'fr', label:'FR'},
        {key:'model_id', label:'Model', render:r=>modelTag(r.model_id)},
...
async function viewLibrary(params, ticket) {
  ...
  const table = new DataTable({
    ...
    columns: [
      ...
      {key:'model_id', label:'Model', render:r=>r.model_id?modelTag(r.model_id):'<span class="unavail">-</span>'},
...
async function viewTable(params, ticket) {
  ...
  const cols = entry.columns.map((label, i) => ({
    ...
    render: i === entry.formula_column ? (r => '<code>' + escapeHTML(String(r[i] ?? '').slice(0, 160)) + '</code>') : (r => {
      ...
      return escapeHTML(displayValue(friendlyValue(v,label), /^(year|Year|İl)$/.test(label)));
    })
  }));
...
async function viewDocument(params, ticket) {
  ...
  if (!entry) { errorView('This document is not part of the public delivery. Use Methodology and sources for the retained public documents.', () => route()); return; }
  loadingView();
...
async function viewNotebook(params, ticket) {
  ...
  if (/FR13/.test(entry.title)) html += '<p class="notice-line"><span class="badge pending">Earlier comparison notebook. The supplied Ministry model is presented separately in Ministry macro.</span></p>';
...
async function viewRequirements(params,ticket){
 loadingView();const [release,review]=await Promise.all([releaseP(),loadAsset(catalogue.review_key)]);if(!fresh(ticket))return;
 ...
 h+='<p class="definition-note">'+bilingual('The tables below inventory available outputs; they do not certify that every requirement has been accepted. ','Aşağıdakı cədvəllər mövcud nəticələri göstərir; bütün tələblərin qəbul edildiyini təsdiqləmir. ')+reviewLink('delivery-scope','Open issues and update procedure','Açıq məsələlər və yeniləmə qaydası')+'</p><details class="panel"><summary>'+bilingual('Non-functional requirements · four system-quality criteria','Qeyri-funksional tələblər · sistemin keyfiyyəti üzrə dörd meyar')+'</summary>'+nfrPanel(review)+'</details>';
 ...
 if(req.id==='FR4')h+=sourceErrorNote();
 view().innerHTML=h;mountNfr(review);
...
function buildSearchIndex() {
  ...
  for (const x of catalogue.tables) searchIndex.push({kind:'table', label:x.title, sub:[x.path,x.description,x.note,(x.columns||[]).join(' ')].filter(Boolean).join(' · '), route:'#/table?id=' + encodeURIComponent(x.id)});
  ...
  for (const g of catalogue.result_groups.filter(g=>!isArchiveScenario(g))) searchIndex.push({kind:'run', label:modelMeta(g.model_id).short + ' · ' + scenarioLabel(g.scenario_id,g.model_id), sub:g.row_count + ' records', route:'#/forecasts?model=' + encodeURIComponent(g.model_id) + '&scenario=' + encodeURIComponent(g.scenario_id)});
...
function providerPanel(e){let h="";
 h+='<section class="panel" id="ownership"><h2>Which system belongs to whom?</h2><p>Both <strong>Ministry macro</strong> and <strong>Ministry CAEM</strong> are supplied Ministry of Economy systems. CAEM means <em>Comprehensive Adaptive Expectations Model</em>; it was developed with IMF technical assistance and customized for the Ministry. The IMF Article IV staff projections are a separate published reference.</p><p>The <a href="'+escapeHTML(e.sources.imf_ta)+'" target="_blank" rel="noopener noreferrer">IMF technical-assistance report</a> explicitly gives the CAEM name on p. 5 and documents its integration on pp. 7, 11 and 15 (published 27 May 2026). Its provenance does not make the Ministry workbook identical to the IMF country team’s Article IV projections.</p><div id="provider-table"></div></section>';
return h;}
...
function cpiPanels(e){let h="";
 h+='<section class="panel" id="cpi-case"><h2>Why does annual-average CPI differ in 2025?</h2><p><code>cpi_infl</code> and <code>cpi_avg</code> are native series names for the same annual-average inflation concept in these mapped rows. The 2025 entries have different information sets and observation statuses.</p><div id="cpi-audit-table"></div><p>The <a href="'+escapeHTML(e.sources.official_cpi)+'" target="_blank" rel="noopener noreferrer">State Statistical Committee’s 14 January 2026 CPI release</a> reports annual-average inflation of <strong>5.6%</strong> and December-on-December inflation of <strong>5.2%</strong>. The Oxlon input cache records the 19 January macroeconomic release; it uses the same annual-average 5.6% value.</p><p>The retained Ministry macro expected value is <strong>3.858 percentage points below</strong> the observed outcome; CAEM’s retained forecast is <strong>0.547 percentage points below</strong>. These are retrospective gaps in saved values. The files do not establish when each retained forecast was formally issued, so they cannot establish a controlled out-of-sample comparison.</p><p>Oxlon’s 2025 value is a historical observation, and the IMF labels 2025 an estimate. Their agreement at 5.6% is <strong>not evidence of forecast superiority</strong>. Retain the supplied model results with their original information date and distinguish them from updated actual data.</p><a class="btn" href="#/overview?concept=cpi_annual_average_inflation&start=2025&end=2031">Compare the complete CPI paths →</a></section>';
...
function imfPanel(e){let h="";
 h+='<section class="panel"><h2>IMF · published staff projections and CAEM method provenance</h2><p>IMF Article IV Table 1 provides a dated external reference. Its CPI sequence for 2025–2031 is <strong>5.6, 6.0, 5.1, 4.1, 4.0, 4.0, 4.0%</strong>. The reference is mapped into the overview for CPI, total GDP growth and non-oil GDP growth; the remaining indicators have no imported IMF mapping.</p><p>The <a href="'+escapeHTML(e.sources.imf_articleiv)+'#page=28" target="_blank" rel="noopener noreferrer">original Table 1</a> is on PDF page 28 (printed page 22), in report 26/112, completed 26 March 2026. The PDF cover is dated 26 May; the <a href="https://www.elibrary.imf.org/view/journals/002/2026/112/002.2026.issue-112-en.xml" target="_blank" rel="noopener noreferrer">IMF eLibrary publication record</a> is dated 27 May 2026. The report’s press-release section contains some different projections; this website retains the identified Table 1 vintage. No separate executable IMF staff model or its coefficients were supplied.</p><p>To study the IMF-assisted CAEM method, use the Ministry CAEM equations and original workbook. To compare published IMF forecasts, use the separate Article IV reference. These are distinct uses of IMF material.</p><a class="btn" href="#/forecasts?model=imf_articleiv_benchmark&scenario=Table1&start=2025&wide=1">IMF reference table →</a></section>';
return h;}
...
function whyNav(active){return '<nav class="topic-nav" aria-label="Why Oxlon pages">'+WHY_PAGES.map(([path,title])=>'<a href="#/'+path+'"'+(path===active?' aria-current="page"':'')+'>'+escapeHTML(uiText(title))+'</a>').join('')+'</nav>'}
...
async function viewDifferences(params,ticket){
 ...
 '<section class="panel"><h2>Appropriate uses of each system</h2><div id="model-use-cases"></div><p>Oxlon and the Ministry macro model both use econometric relationships and bottom-up elements. CAEM also contains estimated or calibrated behavioural equations. The distinction lies in their source inputs, equation systems, estimation and selection rules, accounting closures and intended use.</p>'+whySource('models','Complete model architecture register')+'</section>'+imfPanel(e)+'<section class="panel"><h2>Continue to the evidence</h2>'+whyCards('differences')+'</section>';
...
async function viewOxlonValidation(params,ticket){
 ...
 textTable('oxlon-validation-summary',v.examples,[{key:'indicator',label:'Economic target / error unit',wrap:true,render:r=>escapeHTML(r.indicator)+'<br><small>'+escapeHTML(r.unit)+'</small>'},{key:'method',label:'Method or benchmark',wrap:true},{key:'rmse',label:'RMSE',num:true},{key:'improvement_pct',label:'Error reduction (%)',num:true,render:r=>escapeHTML(r.improvement_pct.toFixed(1))},{key:'target_years',label:'Evaluation sample',wrap:true,render:r=>escapeHTML(r.target_years)+'<br><small>'+r.sample_size+' target years</small>'}], 'Saved one-step assessments · select a row for benchmark error, ensemble weight and interpretation',{onActivate:r=>openDetail(r.indicator+' · '+r.method,propsTable(r))});
...
function bilingual(en,az){return lang==='az'?az:en}
function reviewLink(route,en,az){return '<a class="btn" href="#/'+route+'">'+escapeHTML(bilingual(en,az))+'</a>'}
function sourceErrorNote(){return '<p class="definition-note">'+bilingual('“Recorded source or calculation error” includes an error already stored in the supplied workbook. For example, 8 vərəq.xlsx → 2.4.1.4. → I43 stores #REF! and refers to an invalid source address. This differs from an empty source cell. The original status code and formula remain in record details.','“Qeydə alınmış mənbə və ya hesablama xətası” ilkin iş kitabında saxlanmış xətanı da əhatə edir. Məsələn, 8 vərəq.xlsx → 2.4.1.4. → I43 xanasında #REF! və etibarsız mənbə ünvanına istinad var. Bu, boş mənbə xanasından fərqlidir. İlkin status kodu və düstur qeydin təfərrüatlarında saxlanılır.')+'</p>'}
function reserveWarning(){return '<aside class="review-warning" role="note"><strong>'+bilingual('Reserve-stock path requires review','Ehtiyatların proqnoz trayektoriyası yoxlanmalıdır')+'</strong><p>'+bilingual('The saved CAEM baseline has negative reserve assets in 2027–2029. Reserves/GDP reaches −32.270% in 2029. A negative reserve-asset stock is not a feasible reserve forecast. The retained assumptions and external financing must be reconciled before this path is used for policy analysis.','CAEM-in saxlanmış baza ssenarisində 2027–2029-cu illərdə ehtiyat aktivləri mənfidir. Ehtiyatların ÜDM-ə nisbəti 2029-cu ildə −32,270%-ə çatır. Ehtiyat aktivlərinin mənfi qalığı mümkün ehtiyat proqnozu deyil. Bu trayektoriyadan siyasət təhlilində istifadə etməzdən əvvəl fərziyyələr və xarici maliyyələşmə uzlaşdırılmalıdır.')+'</p>'+reviewLink('caem-checks','Inspect the source formulas and values','Mənbə düsturlarına və qiymətlərə baxın')+'</aside>'}
async function mountReserveEvidence(ticket){const e=await loadAsset(catalogue.review_key);if(!fresh(ticket))return;const h=document.createElement('section');h.className='panel';h.id='reserve-evidence';h.innerHTML='<h2>'+bilingual('External-account feasibility','Xarici sektor hesablarının iqtisadi mümkünlüyü')+'</h2>'+reserveWarning()+'<div id="reserve-records"></div><p>'+bilingual('The reserve-asset stock is accumulated as previous-year assets plus the current reserve change (External USD row 149). Row 150 divides this stock by nominal GDP in USD. The source cache is also negative; the recalculated baseline differs numerically and is shown separately. This locates the problem but does not establish which assumption or upstream flow requires correction.','Ehtiyat aktivləri əvvəlki ilin qalığına cari ilin ehtiyat dəyişikliyinin əlavə edilməsi ilə hesablanır (External USD, sətir 149). Sətir 150 bu qalığı ABŞ dolları ilə nominal ÜDM-ə bölür. İlkin saxlanmış qiymətlər də mənfidir; yenidən hesablanmış baza qiymətləri rəqəmcə fərqlənir və ayrıca göstərilir. Bu, problemin yerini müəyyən edir, lakin hansı fərziyyənin və ya ilkin axının düzəldilməli olduğunu sübut etmir.')+'</p><details><summary>'+bilingual('Source formula trace and cached values','Mənbə düsturları və ilkin saxlanmış qiymətlər')+'</summary><div id="reserve-trace"></div></details>';view().appendChild(h);$('#reserve-records').appendChild(new DataTable({rows:e.external,columns:evidenceColumns(['concept','year','value','unit','source_sheet','source_cell']).map(col=>col.key==='value'?{...col,render:r=>escapeHTML(displayValue(r.value))}:col),tall:false,onActivate:r=>openDetail(r.concept+' · '+r.year,recordDetail(r))}).root);$('#reserve-trace').appendChild(new DataTable({rows:e.reserve_trace,columns:[{key:'year',label:uiText('Year'),get:r=>({Q:2027,R:2028,S:2029})[r.Cell[0]]},{key:'unit',label:uiText('Unit'),get:r=>r.Cell.endsWith('149')?'million USD':'percent GDP'},...['Cell','Original formula','Saved baseline calculation','Source cached value'].map(k=>({key:k,label:k,wrap:true}))],tall:false,onActivate:r=>openDetail(r.Cell,propsTable(r))}).root)}
function nfrPanel(e){return '<section class="panel" id="nonfunctional-requirements"><h2>'+bilingual('Non-functional requirements (NFR1–NFR4)','Qeyri-funksional tələblər (NFR1–NFR4)')+'</h2><p>'+bilingual('Source: MİİS technical specification, §15.5.1, PDF page 38. These assess system quality, data integration, adaptability and presentation; record counts do not establish acceptance.','Mənbə: MİİS texniki tapşırığı, §15.5.1, PDF səhifə 38. Bu tələblər sistemin keyfiyyətini, məlumat inteqrasiyasını, uyğunlaşma qabiliyyətini və təqdimatı qiymətləndirir; qeyd sayı qəbulun sübutu deyil.')+'</p><div id="nfr-table"></div></section>'}
function mountNfr(e){$('#nfr-table').appendChild(new DataTable({rows:e.nfr,columns:[{key:'id',label:bilingual('Reference','İstinad')},{key:lang==='az'?'az':'en',label:bilingual('Requirement','Tələb'),wrap:true},{key:'status',label:bilingual('Evidence status','Sübut vəziyyəti'),wrap:true},{key:lang==='az'?'evidence_az':'evidence',label:bilingual('Available evidence','Mövcud sübut'),wrap:true},{key:lang==='az'?'remaining_az':'remaining',label:bilingual('Remaining work','Qalan iş'),wrap:true}],tall:false}).root)}
async function viewDeliveryScope(params,ticket){loadingView();const [e,r]=await Promise.all([loadAsset(catalogue.review_key),releaseP()]);if(!fresh(ticket))return;let h=head(bilingual('Delivery scope and checks','Təhvilin əhatəsi və yoxlamalar'),bilingual('Available results, unresolved model and data limits, and the process for issuing an updated forecast.','Mövcud nəticələr, həll edilməmiş model və məlumat məhdudiyyətləri, yenilənmiş proqnozun hazırlanması qaydası.'));
h+='<section class="panel"><h2>'+bilingual('What this delivery contains','Təhvilə daxil olanlar')+'</h2><p>'+bilingual('The portal presents the supplied Ministry macro and CAEM systems, saved calculations, source equations and cells, the separate Oxlon model and a dated IMF reference. It supports inspection and comparison of recorded results. The saved examples are not authenticated as the Ministry’s latest official forecasts.','Portal Nazirliyin təqdim etdiyi makro və CAEM sistemlərini, saxlanmış hesablamaları, mənbə tənliklərini və xanalarını, ayrıca Oxlon modelini və tarixli BVF istinadını təqdim edir. Qeydə alınmış nəticələrə baxmaq və onları müqayisə etmək mümkündür. Saxlanmış nümunələrin Nazirliyin ən son rəsmi proqnozları olduğu təsdiqlənməyib.')+'</p>'+reviewLink('delivery','Delivery CSV files and HTML previews','CSV təhvil faylları və HTML cədvəlləri')+reviewLink('ministry-tables','Ministry forecast worksheets','Nazirliyin proqnoz vərəqləri')+'</section>';
h+='<section class="panel"><h2>'+bilingual('Outstanding model and data issues','Model və məlumatlar üzrə açıq məsələlər')+'</h2><div id="delivery-gaps"></div></section>'+nfrPanel(e);
h+='<section class="panel"><h2>'+bilingual('How to update and issue a forecast','Proqnozun yenilənməsi və təqdim edilməsi')+'</h2><ol>'+ (lang==='az'?['Məlumat tarixini və proqnoz dövrünü müəyyən edin. İllik orta və dövr sonu göstəriciləri, qiymət bazasını və sektor əhatəsini razılaşdırın.','Məlumat sahibindən təsdiqlənmiş şirkət, məhsul, sahə, vergi, büdcə və xarici sektor girişlərini alın. Təhvil səhifəsindəki giriş formalarında vahidi, mənbəni, tarixi və ilkin model ünvanını qeyd edin.','Razılaşdırılmış girişləri yerli model icra mühitində hesablayın. Bu ictimai HTML portalı Excel makrolarını və yeni proqnoz hesablamalarını işə salmır.','Hesablama xətalarını, giriş dəyişikliklərini, CAEM bağlanma seçimini, ehtiyatların mümkünlüyünü və milli hesabların uzlaşdırılmasını yoxlayın. Həll edilməmiş məsələləri qeyd edin.','Yeni nəticələri və məlumat manifestini ayrıca versiyada saxlayın; HTML və CSV qiymətlərini tutuşdurun. Əvvəlki versiyanı saxlayın.','Model sahibinin yoxlamasından, Azərbaycan dilində məzmun yoxlamasından və istifadəçi sınağından sonra nəticələrə uyğun təsdiq statusu verin.']:['Agree the information date and forecast horizon, annual-average versus end-period definitions, price bases and sector coverage.','Obtain approved company, product, branch, tax, fiscal and external inputs. Record units, source dates, owners and original model addresses in the input collection forms on the delivery page.','Execute the agreed inputs in the local model environment. This public HTML portal does not run Excel macros or calculate new model forecasts.','Check source errors, input changes, CAEM closure, reserve feasibility and national-account reconciliation. Record unresolved issues.','Save a new version of results and its data manifest; reconcile HTML and CSV values. Preserve the preceding version.','Complete model-owner review, Azerbaijani content review and user acceptance testing before assigning an official approval status.']).map(s=>'<li>'+s+'</li>').join('')+'</ol></section><section class="panel"><h2>'+bilingual('Requirement boundaries','Tələblərin sərhədləri')+'</h2><p>'+bilingual('The supplied §15.5.1 text defines FR1–FR12 and NFR1–NFR4. FR8 concerns population monetary income and expenditure. FR9 specifies annual-average CPI. FR10 specifies annual-average AZN/USD and AZN/EUR. Page 37 does not prescribe quarterly CPI, a numerical MAPE threshold or VAR/ARIMA/GARCH. Any additional booklet criteria need their own source and adoption record. Wage and employment requirements also appear in the separate §15.5.2 module; they should not be silently substituted for §15.5.1 FR8.','Təqdim edilmiş §15.5.1 mətni FR1–FR12 və NFR1–NFR4 tələblərini müəyyən edir. FR8 əhalinin pul gəlirlərinə və xərclərinə, FR9 orta illik istehlak qiymətləri indeksinə, FR10 orta illik AZN/USD və AZN/EUR məzənnələrinə aiddir. Səhifə 37 rüblük İQİ, konkret MAPE həddi və ya VAR/ARIMA/GARCH metodu müəyyən etmir. Əlavə kitabça meyarlarının ayrıca mənbə və qəbul qeydi olmalıdır. Əməkhaqqı və məşğulluq tələbləri ayrıca §15.5.2 modulunda da verilir; bunlar §15.5.1 FR8-in yerinə keçirilməməlidir.')+'</p>'+reviewLink('requirements','Read all 12 functional requirements','Bütün 12 funksional tələbi oxuyun')+'</section>';view().innerHTML=h;
$('#delivery-gaps').appendChild(new DataTable({rows:e.gaps,columns:[{key:lang==='az'?'topic_az':'topic',label:bilingual('Area','Sahə'),wrap:true},{key:lang==='az'?'finding_az':'finding',label:bilingual('Verified limit','Yoxlanmış məhdudiyyət'),wrap:true},{key:lang==='az'?'action_az':'action',label:bilingual('Required next step','Növbəti tələb olunan addım'),wrap:true},{key:'route',label:bilingual('Evidence','Sübut'),render:r=>reviewLink(r.route,'Inspect','Baxın')}],tall:false}).root);mountNfr(e)}
async function viewMinistryTables(params,ticket){loadingView();const sheets=catalogue.tables.filter(t=>t.kind==='native'&&t.book==='8 vərəq.xlsx');const selected=sheets.find(s=>s.sheet===params.get('sheet'))||sheets[0];const all=await loadGroupRows('ministry_macro','baseline');if(!fresh(ticket))return;const rows=all.filter(r=>r.source_file==='8 vərəq.xlsx'&&r.source_sheet===selected.sheet);const map=new Map();for(const r of rows){const key=r.series_id;if(!map.has(key))map.set(key,{meta:r,row:Number(r.source_cell.replace(/[A-Z]/g,'')),years:new Map()});map.get(key).years.set(Number(r.year),r)}const grid=[...map.values()].sort((a,b)=>a.row-b.row);const source=catalogue.tables.find(t=>t.kind==='source-cells'&&t.book==='8 vərəq.xlsx'&&t.sheet===selected.sheet);let h=head(bilingual('Ministry forecast worksheets','Nazirliyin proqnoz vərəqləri'),bilingual('The six supplied “8 vərəq.xlsx” worksheets, in source row order, with saved model values and direct formula links.','Təqdim edilmiş “8 vərəq.xlsx” faylının altı vərəqi: ilkin sətir ardıcıllığı, saxlanmış model qiymətləri və düsturlara birbaşa keçidlər.'));
h+='<p class="definition-note">'+bilingual('This is an HTML view of the supplied template and its saved results, not a newly approved statutory return. 2024 is historical, 2025 expected and 2026–2030 forecast. Missing values and source errors remain visible.','Bu, təqdim edilmiş şablonun və saxlanmış nəticələrin HTML görünüşüdür; yeni təsdiqlənmiş rəsmi hesabat deyil. 2024 tarixi, 2025 gözlənilən, 2026–2030 isə proqnoz dövrüdür. Çatışmayan qiymətlər və mənbə xətaları göstərilir.')+'</p><div class="filterbar">'+selectField(bilingual('Source worksheet','Mənbə vərəqi'),'ministry-sheet',sheets.map(s=>({value:s.sheet,label:s.sheet})),selected.sheet)+'</div><section class="panel"><h2>'+escapeHTML(selected.sheet)+'</h2><div id="ministry-template-results"></div></section>'+sourceErrorNote()+'<div class="actions-row">'+reviewLink('table?id='+selected.id,'Original formulas and evaluation','İlkin düsturlar və hesablama')+(source?reviewLink('table?id='+source.id,'All original cells and headings','Bütün ilkin xanalar və başlıqlar'):'')+reviewLink('delivery','Delivery files','Təhvil faylları')+'</div>';view().innerHTML=h;
const years=[2024,2025,2026,2027,2028,2029,2030];$('#ministry-template-results').appendChild(new DataTable({rows:grid,columns:[{key:'row',label:bilingual('Source row','Mənbə sətri'),num:true},{key:'indicator',label:bilingual('Indicator','Göstərici'),wrap:true,sticky:true,get:r=>r.meta.concept},{key:'unit',label:bilingual('Unit','Vahid'),get:r=>r.meta.unit},...years.map(y=>({key:'y'+y,label:String(y),num:true,get:r=>r.years.get(y)?.value,render:r=>{const v=r.years.get(y);return v?'<span'+(v.status==='run_error'?' class="source-error" title="Recorded source reference error"':'')+'>'+escapeHTML(displayValue(v.value))+(v.status==='run_error'?' †':'')+'</span>':'<span class="unavail">—</span>'}}))],tall:false,caption:bilingual('Mapped numeric output rows in original template order. Select a row for exact values; original headings and all remaining cells are linked below.','Saxlanmış nəticələr ilkin şablon ardıcıllığında. Dəqiq qiymətlər və mənbə xanaları üçün sətri seçin.'),onActivate:r=>openDetail(r.meta.concept,propsTable({source_file:r.meta.source_file,source_sheet:r.meta.source_sheet,unit:r.meta.unit})+Array.from(r.years.values()).map(v=>'<details><summary>'+v.year+' · '+escapeHTML(displayValue(v.value))+'</summary>'+recordDetail(v)+'</details>').join(''))}).root);$('#f-ministry-sheet').addEventListener('change',()=>setParams('ministry-tables',new URLSearchParams({sheet:$('#f-ministry-sheet').value}),true))}

const VIEWS = {
 'delivery-scope':viewDeliveryScope,'ministry-tables':viewMinistryTables,
...
async function route() {
  ...
  breadcrumb(crumbs);
  document.title = 'MİİS 15.5.1 | ' + (section ? section.label() : DETAIL_LABEL[path] || path);
  ...
  try {
    ...
    if(fresh(ticket)){view().dataset.route=buildHash(path,params);if(lang==='az')view().insertAdjacentHTML('afterbegin','<p class="language-scope" lang="az">Azərbaycan dilində interfeys. Analitik mətnlər və mənbə sənədləri qismən ilkin dildə saxlanılır; tam yoxlanmış tərcümə hələ tamamlanmayıb.</p>');}
  } catch (err) {
    if (!fresh(ticket)) return;
