...
async function loadAsset(key) {
  ...
  if (!inFlight.has(key)) inFlight.set(key, new Promise((resolve, reject) => {
    ...
    script.src = catalogue.assets[key].path;
...
const I18N = {
  en: {
    ...
    nav_methods:'Methodology and sources', nav_requirements:'FR1–FR12',
  ...
  az: {
    ...
    nav_methods:'Metodologiya və mənbələr', nav_requirements:'Tələblər FR1-12',
...
const MODEL_META = {
  'ministry_macro': {short:'Ministry', cls:'ministry', colour:'var(--ministry)'},
  'ministry_caem': {short:'CAEM', cls:'caem', colour:'var(--caem)'},
  'oxlon.corrected.20260917': {short:'Oxlon', cls:'oxlon', colour:'var(--oxlon)'},
  'imf_articleiv_benchmark': {short:'IMF Art. IV', cls:'benchmark', colour:'var(--benchmark)'}
};
...
async function viewOverview(params, ticket) {
  ...
  let html=head(lang==='az'?'Makroiqtisadi proqnozlar':'Macroeconomic forecasts',lang==='az'?'MİİS 15.5.1 · Nazirliyin makroekonometrik modeli, CAEM və Oxlon':'MİİS 15.5.1 · Ministry macroeconometric model, CAEM and Oxlon');
  ...
  for(const [m,d,h] of defs)html+='<tr><th scope="row">'+modelTag(m)+'</th><td class="wrap">'+escapeHTML(d)+'</td><td>'+h+'</td><td><a href="#/forecasts?model='+encodeURIComponent(m)+'&scenario=baseline&wide=1&start=2024">'+(lang==='az'?'Cədvəli aç':'Open table')+' →</a></td></tr>';
...
const VIEWS = {
  ...
  methods: viewMethods, document: viewDocument, notebook: viewNotebook, requirements: viewRequirements
};
